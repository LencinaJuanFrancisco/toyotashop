    ...
    <div className="container mt-5">
      ...
      <div>
        ...
        <Link to="/" className="text-decoration-none mx-2" >
          <button className="btn btn-outline-danger">{ cartList.length >0 ? <span>Seguir comprando</span> : <span>Ir a comprar</span>}</button>
        </Link>
      </div>
      { cartList.length >0 ?
        ...
        <tbody>
          {cartList.map((i) => (
            <tr key={i.item.id}>
              ...
              <td>
                <button className="btn btn-outline-secondary" onClick={()=>removeItem(i.item.id)}>Eliminar</button>
              </td>
            </tr>
          ))}
        </tbody>
